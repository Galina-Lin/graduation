 <template>
    <div class="rating_page">
        ...
        <section class="show_time_amount">
        	<section>
                <header class="time_last">订单剩余时间</header>
        ...
        <p class="determine" @click="confrimPay">确认</p>
...
<script>
    ...
    import {setOrderStatus,recharge,setUser} from 'src/service/getData'
    ...
    export default {
      data(){
        ...
        computed: {
            ...mapState([
                'orderMessage', 'userInfo', 'shopid', 'cartPrice', 'orderParam'
            ]),
            //时间转换
            remaining: function (){
        ...
        methods: {
            ...
            remainingTime(){
                ...
                this.timer = setInterval(() => {
                    ...
                    if (this.countNum == 0) {
                        ...
                        this.showAlert = true;
                        this.alertText = '订单超时';
                        setOrderStatus(this.$route.query.orderId||this.orderMessage.id, '支付超时');
                    }
                }, 1000);
            },
            //确认付款
            confrimPay(){
                let show = '下单成功';
                if (this.payWay == 1) {
                    if (this.userInfo.balance - (this.$route.query.total_amount||this.orderMessage.total_amount) < 0) {
                        this.showAlert = true;
                        this.alertText = '余额不足';
                        return;
                    } else {
                        recharge(this.userInfo.user_id, this.userInfo.balance - (this.$route.query.total_amount||this.orderMessage.total_amount));
                    }
                }
                if (this.payWay == 2) {
                }
                console.log(this.$route.query.orderId||this.orderMessage.id,'2323');
                setUser('point', this.userInfo.point + (this.$route.query.total_amount||this.orderMessage.total_amount));
                // this.gotoOrders = true; 
                clearInterval(this.timer);
                setOrderStatus(this.$route.query.orderId||this.orderMessage.id, show);
