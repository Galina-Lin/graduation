...
<!-- 						<ul class="shop_detail_ul">
							<li v-for="item in item.supports" :key="item.id" class="supports">{{item.icon_name}}3</li>
						</ul> -->
					</header>
					...
					<h5 class="fee_distance">
						<p class="fee">
							...
							支持线上支付{{item.piecewise_agent_fee.tips}} 支持线下店主送货上门
						</p>
						<p class="distance_time">
							<span v-if="Number(item.distance)">{{item.distance > 1000? (item.distance/1000).toFixed(2) + 'km': item.distance + 'm'}}
								<span class="segmentation">/</span>
							</span>
							<span class="order_time">{{item.address}}</span>
						</p>
					</h5>
				</hgroup>
			</router-link>
