...
class Admin extends AddressComponent {
	...
	async login(req, res, next){
		...
		form.parse(req, async (err, fields, files) => {
			...
			const {user_name, password, status = 1} = req.query;
	...
	async delAdmin(req, res, next){
		...
		try{
			const admins = await AdminModel.findOne({user_name: user_name});
			await admins.remove()
			res.send({
				status: 1,
				success: '删除管理员成功',
