...
class Order extends BaseComponent{
	...
	async postOrder(req, res, next){
		...
		form.parse(req, async (err, fields, files) => {
			...
			try{
				const orderInfo = await OrderModel.create(orderObj);
				res.send({
					status: 1,
					success: '下单成功，请及时付款',
	...
	async statusBarOrder(req, res, next){
		const {order_id, status} = req.query;
		try{
			const orders = await OrderModel.findOne({id: order_id});
			orders.status_bar.title = status;
			orders.save();
			res.send({
				status: 1,
				message: 'success',
